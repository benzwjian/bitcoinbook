...
----
```
This locking script is created by Alice. It identify this output that becomes UTXO in future belongs to Bob. After this transaction is mined, the Bob's wallet will view it as a spendable balance by <Cafe Public Key Hash>. 
```
...
----
```
After Bob receive a UTXO created by Alice, he decides to spend it. Bob needs to create a unlocking script in an input of new transaction. Bitcoin system will validate the transaction by <Cafe Signature> <Cafe Public Key> combined with locking script.
```
